...
/**
 * Gets the a list of user's a user is following
 *
 * @param {Number} from (optional) the start index. defaults to 0.
 * @param {Number} length (optional) number of user IDs to get. defaults to 100.
 * @param {Function} fn callback function
 * @api public
 */

Spotify.prototype.followingList = function (user, from, length, fn) {
  // argument surgery
  if ('function' == typeof user) {
    fn = user;
    from = length = user = null;
  ...
  debug('followingList(%j, %j, %j)', user, from, length);

  var self = this;
  var hm = 'hm://socialgraph/subscriptions/user/' + user + '?from=' + from + '&length=' + length;
  ...
  this.sendProtobufRequest({
    ...
    responseSchema: MercuryMultiGetRequest
  }, fn);
};

/**
 * Gets the a list of user's that are following a user
 *
 * @param {Number} from (optional) the start index. defaults to 0.
 * @param {Number} length (optional) number of user IDs to get. defaults to 100.
 * @param {Function} fn callback function
...
Spotify.prototype.followerList = function (user, from, length, fn) {
  // argument surgery
  if ('function' == typeof user) {
    fn = user;
    from = length = user = null;
  ...
  debug('followerList(%j, %j, %j)', user, from, length);

  var self = this;
  var hm = 'hm://socialgraph/subscribers/user/' + user + '?from=' + from + '&length=' + length;
